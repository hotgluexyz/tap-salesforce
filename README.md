...
The tap selects the OAuth grant based on the config. When `refresh_token` is provided it uses the `refresh_token` grant. When `refresh_token` is omitted it uses the client credentials grant. For client credentails, a `"domain"` set to your org's My Domain URL (e.g. `"https://mycompany.my.salesforce.com"`) is required, since Salesforce only supports the client credentials flow on the org's My Domain.
